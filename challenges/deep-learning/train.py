...
def train(train_loader, validate_loader, processor, model, n_epochs, lr):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print(
            f"Epoch {epoch}\t|\tTrain Loss {train_loss:.4f}\tTrain Accuracy {train_accuracy:.4f}\tTest Loss {validate_loss:.4f}\tTest Accuracy {validate_accuracy:.4f}"
        )

        if epoch % 10 == 0:
            torch.save(
                model.state_dict(),
                os.path.join(".", "saved_models", "epoch_{}.pt".format(epoch)),
            )
    ...
    torch.save(
        model.state_dict(),
        os.path.join(".", "saved_models", "final_{}.pt".format(datetime.now())),
    )
...
def main():
    ...
    if args.train:
        ...
        test_dataset = RecaptchaDataset(args.dataset_path, "test")
